...
def get_preview_image_or_video_by_model_path(model_path: str, max_size_preview, skip_nsfw_preview):
    if not model_path:
        util.printD("model_path is empty")
        return
    ...
    first_preview = base + ".png"
    sec_preview = base + ".preview.png"
    video_preview = base + ".preview.mp4"
    info_file = base + suffix + model.info_ext

    # check preview image or video
    if not os.path.isfile(sec_preview) and not os.path.isfile(video_preview):
        # need to download preview image or video
        util.printD("Checking preview media for model: " + model_path)
        # load model_info file
        if os.path.isfile(info_file):
            model_info = model.load_model_info(info_file)
            if not model_info:
            ...
            if "images" in model_info.keys():
                if model_info["images"]:
                    for img_dict in model_info["images"]:
                        ...
                        # Check if the preview is a video or an image
                        if preview_type == "image":
                            if "url" in img_dict.keys():
                                img_url = img_dict["url"]
                                if max_size_preview:
                            ...
                            if "url" in img_dict.keys():
                                video_url = img_dict["url"]
                                util.download_file(video_url, video_preview)
                                # we only need 1 preview video
                                break
